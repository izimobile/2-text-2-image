...
function generateImage(content, config, fileName, leftMarginForLogo) {
  var conf = _.defaults(config, defaults);

  var textData = createTextData(content, conf.maxWidth - conf.margin, conf.fontSize, conf.lineHeight, conf.bgColor, conf.textColor, conf.fontFamily, leftMarginForLogo);
  ...
  if (conf.debug) {
    return new Promise(function (resolve, reject) {
      ...
      var out = fs.createWriteStream(path.join(process.cwd(), fileName || 'test' + '.png'));
...
function createTextData(text, maxWidth, fontSize, lineHeight, bgColor, textColor, fontFamily, leftMarginForLogo) {
  // create a tall context so we definitely can fit all text
  var textCanvas = new Canvas(maxWidth, 1000);
  var  textContext = textCanvas.getContext('2d');
  var  textX = leftMarginForLogo? maxWidth/5: 0;
  var  textY = 0;
